-- Transparência Federal — materialized view de cruzamento ALEPE × Câmara × ALESP
--
-- Substitui a view regular (lenta: full scan em 670k + 1,4M linhas a cada request).
-- Materialized view é populada na criação e via função refresh_fornecedores_intersetados().
-- Refresh: chamar após cada ingestão mensal de qualquer das três casas.

-- 1. Remover view regular criada na migration anterior
DROP VIEW IF EXISTS public.fornecedores_intersetados CASCADE;
...
-- 3. Índices
CREATE UNIQUE INDEX fornecedores_intersetados_cnpj_idx
  ON public.fornecedores_intersetados (cnpj);

CREATE INDEX fornecedores_intersetados_total_idx
  ON public.fornecedores_intersetados (total_geral DESC);

CREATE INDEX fornecedores_intersetados_n_casas_idx
  ON public.fornecedores_intersetados (n_casas DESC);

-- 4. Função de refresh (chamada após cada ingestão)
...
COMMENT ON MATERIALIZED VIEW public.fornecedores_intersetados IS
  'Fornecedores (CNPJ) presentes na ALEPE e em ≥1 outra casa (ALESP ou Câmara Federal). '
  'Chamar refresh_fornecedores_intersetados() após cada ingestão mensal.';
