...
import { getDespesaDeputado, getCeapNotas, type CeapNota } from "~/services/despesas";
...
export default async function ExpensaDeputadoPage({ params, searchParams }: Props) {
  const { id } = await params;
  ...
  // Notas detalhadas (paginação interna pra superar 1000 do Supabase)
  const todasNotas = await getCeapNotas(id, 3000).catch(() => [] as CeapNota[]);

  // Aplica filtros (ano / categoria)
  ...
  // ── Agregações ────────────────────────────────────────────────
  const totalLiquido = notas.reduce((s, n) => s + (Number(n.valor_liquido) || 0), 0);
  const totalGlosa = notas.reduce((s, n) => s + (Number(n.valor_glosa) || 0), 0);
  ...
  for (const n of notas) {
    ...
    cur.total += Number(n.valor_liquido) || 0;
    cur.qtd++;
    catMap.set(k, cur);
  }
  ...
  // Por mês (só faz sentido se tem 1 ano selecionado, mas mostro sempre)
  ...
  // Universo de anos (das notas)
  const anosDisp = Array.from(new Set(todasNotas.map((n) => n.ano)))
    .sort((a, b) => b - a);

  const qtdFornecedores = fornMap.size;
  const qtdCategorias = catMap.size;

  // URLs com filtros preservados
    ...
    <>
      ...
      >
        <div className="container" style={{ padding: "2rem 1.5rem 1.5rem", maxWidth: "1080px" }}>
          ...
          >
            <Link href="/expenses" style={{ color: "hsl(var(--primary))", textDecoration: "none" }}>
              Despesas CEAP
            </Link>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: "0.75rem", flexWrap: "wrap" }}>
            <h1 style={{ fontSize: "1.625rem", margin: 0, lineHeight: 1.2 }}>{dep.nome}</h1>
            {dep.sigla_partido && (
      ...
      <div className="container" style={{ padding: "1.5rem 1.5rem 4rem", maxWidth: "1080px" }}>
        {/* Filtros */}
        {(anosDisp.length > 0 || porCategoria.length > 0) && (
          <>
            {anosDisp.length > 0 && (
        ...
        <div className="bloomberg-kpi-grid" style={{ marginBottom: "1.5rem" }}>
          <Kpi label="Total líquido" value={fmtBRL(totalLiquido)} sub={anoFiltro ? `ano ${anoFiltro}` : "todos os anos"} />
          <Kpi label="Notas fiscais" value={fmtN(totalNotas)} sub={totalGlosa > 0 ? `glosa: ${fmtBRL(totalGlosa)}` : "sem glosa"} />
          ...
          >
            <p style={{ fontSize: "0.9375rem" }}>
              {anoFiltro || catFiltro
                ...
                : "Sem notas fiscais registradas para este deputado."}
            </p>
            {(anoFiltro || catFiltro) && (
              <Link href={`/expenses/${id}`} style={{ fontSize: "0.875rem", color: "hsl(var(--primary))", marginTop: "0.75rem", display: "inline-block" }}>
                Limpar filtros
              </Link>
            )}
          </div>
                  ...
                  <tbody>
                    {porCategoria.slice(0, 10).map(({ cat, total, qtd }) => (
                      <tr key={cat}>
                        ...
                        <td style={{ textAlign: "right", color: "hsl(var(--text-caption))", fontSize: "0.75rem" }}>
                          {totalLiquido > 0 ? ((total / totalLiquido) * 100).toFixed(1) : "0"}%
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="bloomberg-card" style={{ padding: 0, overflow: "hidden" }}>
              <div style={{ padding: "1rem 1.25rem 0.75rem" }}>
                <h3 style={{ ...sectionTitle, margin: 0 }}>
                  Notas fiscais{" "}
                  <span style={{ fontWeight: 400, color: "hsl(var(--text-caption))" }}>
                    ({fmtN(totalNotas)} registros · clique no número do documento pra abrir o PDF original)
                  </span>
                </h3>
              </div>
              <div style={{ overflowX: "auto" }}>
                <table className="bloomberg-table" style={{ width: "100%", minWidth: "920px" }}>
                  <thead>
                    <tr>
                      <th>Data</th>
                      <th>Fornecedor</th>
                      ...
                      <tr key={n.id}>
                        ...
                        <td
                          style={{
                            fontSize: "0.75rem",
                            fontFamily: "var(--font-mono)",
                            color: n.url_documento ? "hsl(var(--primary))" : "hsl(var(--text-caption))",
                          }}
                        >
                          {n.url_documento ? (
                            <a
                              ...
                              title="Abrir PDF do recibo"
                            >
                              {n.num_documento || "ver PDF"} ↗
                            </a>
                          ) : (
                            n.num_documento || "—"
                          )}
              ...
              {totalNotas > 500 && (
                ...
                >
                  Exibindo 500 de {fmtN(totalNotas)} notas (mais recentes primeiro).
                  Use os filtros de ano/categoria para refinar.
                </p>
              )}
            </div>
          </>
        ...
        {dep.historico.length > 0 && (
          <div className="bloomberg-card" style={{ marginTop: "1.25rem" }}>
            <h3 style={sectionTitle}>Posição no ranking CEAP por ano</h3>
            <table className="bloomberg-table" style={{ width: "100%" }}>
              <thead>
                <tr>
                  <th>Ano</th>
        ...
        >
          Fonte: Câmara dos Deputados — API de dados abertos (CEAP). Notas fiscais
          referem-se a despesas reembolsadas pela Cota para Exercício da Atividade
          Parlamentar. Cada PDF abre direto no portal da Câmara.
          Valores em R$ nominais, líquidos de glosa.
        </p>
      </div>
    </>
  );
