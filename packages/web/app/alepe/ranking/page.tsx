/**
 * Ranking de verba indenizatória — deputados ALEPE.
 * Rota: alepe.thebrinsider.com/ranking → /alepe/ranking
 *
 * ALEPE tem 163 parlamentares históricos (49 ativos + 114 de legislaturas anteriores)
 * e dados desde jan/2015. A view alepe_verba_resumo_mensal inclui o flag `ativo`
...
export const metadata: Metadata = {
  title: "Ranking de Gastos — ALEPE | The BR Insider",
  description:
    "Deputados estaduais de Pernambuco ordenados por verba indenizatória reembolsada. 11 anos de histórico — 2015 a hoje, nota a nota, com CNPJ do fornecedor.",
  alternates: { canonical: "https://alepe.thebrinsider.com/ranking" },
  openGraph: {
    title: "Ranking ALEPE — Verba Indenizatória",
    description: "163 deputados, 11 anos de histórico, nota a nota, CNPJ a CNPJ.",
    url: "https://alepe.thebrinsider.com/ranking",
...
function filtrar(rows: ViewRow[], periodo: Periodo, soAtivos: boolean): ViewRow[] {
  ...
  if (periodo === "atual") {
    ...
    out = out.filter(
      (r) => r.ano === anoAtual || (r.ano === anoAtual - 1 && r.mes > mesAtual),
    );
...
function agregar(rows: ViewRow[]): DeputadoAgg[] {
  ...
  for (const r of rows) {
    const existing = map.get(r.id_alepe);
    if (!existing) {
      map.set(r.id_alepe, {
        id_alepe: r.id_alepe,
        nome: r.nome,
...
export default async function AlepeRankingPage({
  searchParams,
}: {
    ...
    <>
      ...
      >
        <div className="container" style={{ padding: "2rem 1.5rem 1.5rem", maxWidth: "960px" }}>
          ...
          >
            Gastos reembolsados pela ALEPE nota a nota, com CNPJ do fornecedor e data exata.
            Fonte: API PHP da assembleia (sem autenticação).
          </p>
        </div>
      </section>
      ...
      <div className="container" style={{ padding: "1.5rem 1.5rem 4rem", maxWidth: "960px" }}>
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  border: `1px solid ${
                    periodo === f ? "hsl(var(--primary))" : "hsl(var(--border))"
                  }`,
                  borderRadius: "4px",
                  color: periodo === f ? "hsl(var(--primary))" : "hsl(var(--text-body))",
                  backgroundColor:
                    periodo === f ? "hsl(var(--primary) / 0.08)" : "transparent",
                  textDecoration: "none",
        ...
        <div className="bloomberg-kpi-grid" style={{ marginBottom: "1.5rem" }}>
          <Kpi label="Total reembolsado" value={fmtBRL(totalGasto)} />
          <Kpi label="Notas fiscais" value={fmtNum(totalNotas)} />
          <Kpi
            label="Deputados c/ gastos"
            value={`${deputadosComGasto}${soAtivos ? "/49" : ""}`}
          />
          <Kpi
        ...
        >
          ...
          {incluirHistoricos && " · incluindo deputados de legislaturas anteriores"}. Fonte: ALEPE
          — API PHP de transparência (4 endpoints, sem autenticação). Atualizado mensalmente.
          Valores em R$ nominais. Notas com data exata e CNPJ do fornecedor disponíveis no detalhe.
        </p>
      </div>
    </>
  );
