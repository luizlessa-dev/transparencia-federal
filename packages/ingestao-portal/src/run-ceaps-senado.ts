...
import { jobIngestaCeapsSenado } from "./job-ingestao-ceaps-senado.js";
...
console.log("▶ Iniciando job_ingestao_ceaps_senado...");
if (anos) console.log(`  Anos: ${anos.join(", ")}`);
else console.log("  Anos: 2019–2025 (padrão)");

const resultado = await jobIngestaCeapsSenado({
  supabaseUrl: url,
...
for (const r of resultado.resultados_por_ano) {
  ...
  console.log(`  ${r.ano}: ${r.total} despesas, ${r.senadores} senadores — ${r.duracao_ms}ms${flag}`);
}
